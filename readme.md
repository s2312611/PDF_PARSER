[日本語](#可搬型文書型式解析器)｜[English](#portable-document-format-parser)

# 可搬型文書型式解析器

論文を翻訳する際に、可搬型文書型式（`.pdf`）よりテキスト型式（`.txt`）が便利な場合が有る。其処で、可搬型文書型式を解析して、文章の部分を抽出できるプログラムを作成した。

キーワード：可搬型文書型式（`.pdf`）、解析器。
